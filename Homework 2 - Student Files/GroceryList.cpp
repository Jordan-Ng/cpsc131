...
std::size_t GroceryList::find( const GroceryItem & groceryItem ) const
{
  ...
  auto position = std::find(_gList_vector.begin(), _gList_vector.end(), groceryItem);
  if (position != _gList_vector.end()){
    std::cout << (*position).brandName() << std::endl;
    return std::distance(_gList_vector.begin(), position);
  }
  return size();
  /////////////////////// END-TO-DO (2) ////////////////////////////
}
...
void GroceryList::insert( const GroceryItem & groceryItem, std::size_t offsetFromTop )        // insert provided grocery item at offsetFromTop, which places it before the current grocery item at offsetFromTop
{
  ...
  { /**********  Part 1 - Insert into array  ***********************/
    ///////////////////////// TO-DO (4) //////////////////////////////
      ...
      for (auto i = _gList_array.size(); i != offsetFromTop; i--){        
        _gList_array.begin()[i] = std::move(_gList_array.begin()[i-1]);
      }
      _gList_array.begin()[offsetFromTop] = std::move(groceryItem);
      _gList_array_size++;
  ...
  { /**********  Part 3 - Insert into doubly linked list  **********/
    ...
    _gList_dll.push_back(groceryItem);
  ...
  { /**********  Part 4 - Insert into singly linked list  **********/
    ...
    auto offSetFromTop_to_iterator = std::next(_gList_sll.begin(), offsetFromTop-1);
    _gList_sll.insert_after(offSetFromTop_to_iterator, groceryItem);
